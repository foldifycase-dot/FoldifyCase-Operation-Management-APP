  ...
  if (service === "shopify") {
    ...
    if (action === "shipping-labels") {
      ...
      try {
        ...
        return res.status(200).json({ labels: {}, count: 0, error: err.message });
      }
    }
...
async function callComposio(toolSlug, args, apiKey, connectedAccountId, userId) {
  // Composio v3.1 requires user_id whenever connected_account_id is used,
  // and the user_id must equal the owner of that connected account.
